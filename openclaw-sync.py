...
def fingerprint_dir(root: Path) -> str:
    ...
    for path in sorted(p for p in root.rglob("*") if p.is_file()):
        ...
        try:
            ...
            # Volatile files can disappear/change while syncing; skip them
            # so one transient read failure does not abort the entire sync loop.
            continue
    return hasher.hexdigest()
...
def create_snapshot_dir(source_root: Path) -> Path:
    ...
    for path in sorted(source_root.rglob("*")):
        ...
        try:
            ...
            # Skip files that are being rotated/deleted mid-snapshot.
            continue
    return staging_root
